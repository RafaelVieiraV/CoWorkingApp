...
public class WorkspaceServiceImplTest {
    ...
    // FUNCIÓN: buscar con nombre -> usa searchWorkspaces con el nombre
    @Test
    void searchPage_withName_shouldReturnResults() {
        Pageable pageable = PageRequest.of(0, 10);
        Page<Workspace> page = new PageImpl<>(List.of(buildWorkspace(10L, "Escritorio 1", WorkspaceType.ESCRITORIO)));
        Mockito.when(workspaceRepository.searchWorkspaces(
                        ArgumentMatchers.eq("Escri"), ArgumentMatchers.isNull(),
                        ArgumentMatchers.isNull(), ArgumentMatchers.eq(pageable)))
                .thenReturn(page);

        Page<WorkspaceResponseDto> result = workspaceService.searchPage("Escri", null, null, pageable);

        Assertions.assertEquals(1, result.getTotalElements());
        Mockito.verify(workspaceRepository).searchWorkspaces(
                ArgumentMatchers.eq("Escri"), ArgumentMatchers.isNull(),
                ArgumentMatchers.isNull(), ArgumentMatchers.eq(pageable));
    }

    // FUNCIÓN: buscar sin filtros -> usa searchWorkspaces con nombre null
    @Test
    void searchPage_withoutName_shouldReturnResults() {
        Pageable pageable = PageRequest.of(0, 10);
        Page<Workspace> page = new PageImpl<>(List.of(buildWorkspace(10L, "Escritorio 1", WorkspaceType.ESCRITORIO)));
        Mockito.when(workspaceRepository.searchWorkspaces(
        ...
        Page<WorkspaceResponseDto> result = workspaceService.searchPage(null, null, null, pageable);
